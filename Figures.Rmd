...
ggplot(Common_Events[which(sapply(Common_Events$Identifier,function(x){strsplit(x,"_")[[1]][1]})=="SE"),],aes(x = Pan,y = Ov,color=Color,fill = Color))+
  ...
  geom_text_repel(data = sig_up_genes,aes(label = "MapK3\n(ERK1)"),force = 2,nudge_x = -0.2,nudge_y = 0.1,colour = "black",size=3)+
  theme_classic()+
...
phyper(ov,gene_set_1,universe-gene_set_1,gene_set_2,lower.tail=F)
...
Pan<-list("DE"=Pan_GE$Genes,"AS"=unique(Pancreatic_Events_sig$GeneID))
...
#Calcualting significance
ov<-length(intersect(Pan_GE$Genes,Pancreatic_Events_sig$GeneID))
gene_set_1<-length(Pan_GE$Genes)
...
#Making a list of DE and AS Genes
Ov<-list("DE"=Ov_GE$Genes,"AS"=unique(Ovarian_Events_sig$GeneID))
...
#Calcualting significance
ov<-length(intersect(Ov_GE$Genes,Ovarian_Events_sig$GeneID))
gene_set_1<-length(Ov_GE$Genes)
